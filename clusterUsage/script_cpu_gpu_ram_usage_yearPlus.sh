...
start_day=$(date --date "-1 year" +\%Y)-$(date --date "-1 month" +\%m)-$(date --date "-27 day" +\%d)
end_day=$(date +\%Y)-$(date +\%m)-$(date --date "-1 days" +\%d)

/software/miniconda3/bin/python $basePath$python_code $start_day $end_day > "${basePath}${start_day}_${end_day}_gpu_cpu_ram_usage_custom"
