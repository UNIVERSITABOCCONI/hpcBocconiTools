...
start=$(date --date "-1 year" +\%Y)-$(date --date "-2 month" +\%m)-$(date --date "-2 day" +\%d)
...
sacct --noheader -Xa --starttime ${start}T00:00:00 --endtime ${end}T00:00:00 -o jobid,state,user,partition,elapsed,submit,start | grep -v gpu > waiting_time_cpu_${start}_${end}.txt
