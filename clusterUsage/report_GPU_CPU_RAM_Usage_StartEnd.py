...
while (start_date <= end_date):
	...
	proc = subprocess.Popen(["sreport cluster Utilization --tres=\'gres/gpu,cpu,mem\' -t Percent start=" + start_date.strftime(date_format) + " end=" + (start_date+delta).strftime(date_format) + " -n | awk '{ print $3 }'"], stdout=subprocess.PIPE, shell=True)
